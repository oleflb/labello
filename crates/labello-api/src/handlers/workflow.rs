...
use labello_domain::{
    AnnotationGeometry, AnnotationSource, AnnotationType, DatasetId, DatasetRole, EventPayload,
    ImageId, KeybindingSet, OfflineSyncRequest, PrelabelSuggestion,
};
...
pub(crate) async fn append_event(
    ...
    let repo = state.repo(&dataset_id);
    let metadata = repo.load_dataset().await?;
    ensure_dataset_role(&metadata, &actor, actor.role.clone())?;
    validate_payload(&metadata, &image_id, &request.payload)?;
    Ok(Json(
        repo.append_payload(&image_id, &actor, request.payload)
            .await?,
    ))
}
...
pub(crate) async fn record_review(
    ...
    Ok(Json(
        repo.append_payload(&image_id, &actor, EventPayload::ReviewRecorded { review })
            .await?,
    ))
}
...
pub(crate) async fn record_correction(
    ...
    Ok(Json(repo.append_payload(&image_id, &actor, payload).await?))
}
